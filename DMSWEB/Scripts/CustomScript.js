...
function BindChildListForParent(cururl, $ctrl1, parentID, defaultOption) {
    $.ajax({
        url: cururl,
        dataType: "json",
        type: "POST",
        ...
        success: function (data) {
            if (data.success) {
                BindDropdownWithValues(data.message1, $ctrl1, defaultOption);
            }
        },
        error: function (xhr) {
            alert('error ' + xhr.message);
...
function BindDropdownWithValues(data, ctrl, defaultOption) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].FolderName != undefined && data[i].FolderName != null) {
            Folder = data[i].FolderName;
        }
        if (data.length == 1)
            ctrl.append('<option selected="selected" folder="' + Folder +'"  level="' + Level +'"  code="' + code + '" title="' + data[i].Name + '" value="' + data[i].ID + '">' + data[i].Name + '</option>');
        else
            ctrl.append('<option folder="' + Folder +'"  level="' + Level +'"  code="' + code + '" title="' + data[i].Name + '" value="' + data[i].ID + '">' + data[i].Name + '</option>');
    }
}

function BindDropDownBoxesForNewUpload(cururl, $ctrl1, $ctrl2, $ctrl3, $ctrl4, $ctrl5, defaultOption) {
    $.ajax({
        url: cururl,
        dataType: "json",
        type: "POST",
        ...
        success: function (data) {            
            if (data.success) {
                if ($ctrl1 != '')
                    BindDropdownWithValues(data.message1, $ctrl1, defaultOption);
                if ($ctrl2 != '')
                    BindDropdownWithValues(data.message2, $ctrl2, defaultOption);
                if ($ctrl3 != '')
                    BindDropdownWithValues(data.message3, $ctrl3, defaultOption);
                if ($ctrl4 != '')
                    BindDropdownWithValues(data.message4, $ctrl4, defaultOption);
                if ($ctrl5 != '')
                    BindDropdownWithValues(data.message5, $ctrl5, defaultOption);
            }
        },
        error: function (xhr) {
            alert('error ' + xhr.message);
...
function BindModelPartsForSmallCat(cururl, $ctrl1, $ctrl2, parentID, defaultOption) {
    $.ajax({
        url: cururl,
        dataType: "json",
        type: "POST",
